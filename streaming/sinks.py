...
class FFmpegSink:
    ...
    def __init__(self, width, height, fps, target,
                 ...
                 sdp_path="stream.sdp", verbose=False):
        self.width = width
        self.height = height
        self.fps = fps
        self.target = target
        ...
        if target.startswith("rtp://"):
            # Repeat SPS/PPS with every keyframe so a receiver that joins late,
            # or reuses an older SDP, can still sync on its own.
            ...
            cmd += ["-f", "mpegts", "-flush_packets", "1",
                    _with_query(target, f"pkt_size=1316&buffer_size=2000000")]
        else:
